...
class StandardBusinessDocumentGeneratorTest {
    ...
    fun `given a valid document, should generate valid SBD`() {
        val (header, document) =
            sbdGenerator.generateStandardBusinessDocument<OrderType>(
                "/outbound/outbound-valid-order-no-sbdh.xml".getResource()
    ...
    fun `given a valid document with v3 scheme ID, should return a valid SBD`() {
        val (header, document) =
            sbdGenerator.generateStandardBusinessDocument<OrderType>(
                "/outbound/outbound-valid-order-schemeid-v3-no-sbdh.xml".getResource()
            ).getOrElse { throw IllegalStateException("should not return error") }
    ...
    fun `given an invalid payload, should return parse error message`() {
        sbdGenerator.generateStandardBusinessDocument<OrderType>(
            "/outbound/outbound-invalid-payload.json".getResource()
        ).getErrorOrElse { throw IllegalStateException("should return error") }
            .`should equal`(ErrorMessage.StandardBusinessDocument.FailedToParseDocumentType)
    }

    @Test
    fun `given an unhandled document type, should return invalid document message`() {
        sbdGenerator.generateStandardBusinessDocument<ForecastType>(
            "/outbound/outbound-valid-invoice-no-sbdh.xml".getResource()
        ).getErrorOrElse { throw IllegalStateException("should return error") }
            .`should equal`(ErrorMessage.StandardBusinessDocument.InvalidDocumentType)
    }

    @Test
    fun `given an invalid document with missing required values, should return error message`() {
        sbdGenerator.generateStandardBusinessDocument<OrderType>(
            "/outbound/outbound-invalid-order-missing-required-values-no-sbdh.xml".getResource()
        ).getErrorOrElse { throw IllegalStateException("should return error") }
            .`should equal`(ErrorMessage.StandardBusinessDocument.MissingRequiredValuesFromDocument)
    }

    @Test
    fun `given a document with an invalid scheme ID, should return error message`() {
        sbdGenerator.generateStandardBusinessDocument<OrderType>(
            "/outbound/outbound-invalid-order-invalid-schemeid-no-sbdh.xml".getResource()
        ).getErrorOrElse { throw IllegalStateException("should return error") }
            .`should equal`(ErrorMessage.StandardBusinessDocument.InvalidSchemeIdForParticipant)
    }
}
