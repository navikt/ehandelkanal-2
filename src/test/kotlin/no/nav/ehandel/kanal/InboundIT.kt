...
class InboundIT {
    ...
    private val mockEndpoints = listOf(accessPointClient, inboxQueue, inboundRoute,
    ...
    fun `valid catalogue with size smaller than set limit`() {
        ...
        NotifyBuilder(camelContext).fromRoute(ACCESS_POINT_READ.id).whenExactlyCompleted(1).create().matches(3, TimeUnit.SECONDS)
    ...
    fun `invalid message with not well formed XML`() {
        ...
        ebasysUnknownFiles.run {
            expectedPropertyReceived(InboundSbdhMetaDataExtractor.CAMEL_XML_PROPERTY, false) // Expect that the XMLDetector has set the property
            expectedMessageCount(1) // Expect that the message is sent to Ebasys (manual processing folder)
        }
        NotifyBuilder(camelContext).fromRoute(INBOUND_EHF.id).whenExactlyCompleted(1).create().matches(3, TimeUnit.SECONDS)
        verify(exactly(0), postRequestedFor(urlEqualTo(juridiskLoggUrl)))
        assertMockEndpointsSatisfied()
    }
    ...
    private fun setUpInboundValidExpectations() {
        ...
        inboundLogger.run {
            ...
            expectedPropertyReceived(InboundSbdhMetaDataExtractor.CAMEL_XML_PROPERTY, true) // Expect that the XMLDetector has validated the file
        }
    }
    ...
    private fun verifyAccessPointRequests() {
        verify(exactly(1), getRequestedFor(urlEqualTo(inboxMessagesUrl))
        ...
        verify(exactly(1), getRequestedFor(urlEqualTo(inboxMessageXmlDocUrl))
            .withHeader(AccessPointProps.messages.header, equalTo(AccessPointProps.messages.apiKey))
            .withBasicAuth(BasicCredentials(ServiceUserProps.username, ServiceUserProps.password))
            .withHeader(HttpHeaders.Accept, equalTo(ContentType.Application.Xml.toString())))

        verify(exactly(1), postRequestedFor(urlEqualTo(inboxReadUrl))
            .withHeader(AccessPointProps.inbox.header, equalTo(AccessPointProps.inbox.apiKey))
            .withBasicAuth(BasicCredentials(ServiceUserProps.username, ServiceUserProps.password))
            .withHeader(HttpHeaders.Accept, equalTo(ContentType.Application.Xml.toString())))
    }
